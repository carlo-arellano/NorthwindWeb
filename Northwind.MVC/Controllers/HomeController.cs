...
namespace Northwind.MVC.Controllers
{
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            byte[]? cachedValueBytes = await _distributedCache.GetAsync(CategoriesKey);
        ...
        public async Task<IActionResult> ProductDetail(int? id, string alertstyle = "success")
        {
            ...
            if (!_memoryCache.TryGetValue($"{ProductKey}{id}", out Product? model))
            {
                ...
                model = await _db.Products.Include(p => p.Category).SingleOrDefaultAsync(p => p.ProductId == id);
        ...
        private async Task<List<Category>> GetCategoriesFromDatabaseAsync()
        {
            ...
            await _distributedCache.SetAsync(CategoriesKey, cachedValueBytes, cacheEntryOptions);
